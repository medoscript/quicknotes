...
public class User implements UserDetails { // UserDetails
    ...
    @ManyToMany(fetch = FetchType.EAGER)    // жаджетная загрузка
    @JoinTable(
            name = "user_role",                                  //таблица связи
            joinColumns = @JoinColumn(name = "user_id"),         //внешний ключ
            inverseJoinColumns = @JoinColumn(name = "role_id")   //внешний ключ
    )
    private Set<Role> roles; //коллекция ролей
